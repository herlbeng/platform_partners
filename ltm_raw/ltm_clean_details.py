...
for filename in os.listdir(input_folder):
    if filename.endswith('.csv'):
        ...
        clean_filename = filename.replace('_', '').replace('LTM', '').replace('KPIS', '').replace('KPI', '').replace('DATA', '').replace('CENTER', '').replace('2023', '').replace('2024', '').replace('2025', '').replace('&', ' AND ')
        final_filename = re.sub(r"\s", "_", clean_filename).upper()
        output_path = os.path.join(output_folder, final_filename)
        
        # Cargar archivo CSV
        #df = pd.read_csv(input_path, header=None)
        df = pd.read_csv(input_path, header=None, skiprows=6, nrows=24)  # Saltar las primeras 6 filas y leer 24 filas
        ...
        # Conserva las líneas necesarios: 0:6-29
        #df = pd.concat([df.iloc[[0]], df.iloc[6:30]], ignore_index=True)
